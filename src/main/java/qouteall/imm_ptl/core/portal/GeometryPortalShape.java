...
public class GeometryPortalShape {
    ...
    public static @Nullable Mesh2D readOldMeshFromTag(ListTag tag) {
        ...
        for (int i = 0; i < triangleNum; i++) {
            mesh.addTriangle(
                tag.getDoubleOr(i * 6 + 0, 0.0D),
                tag.getDoubleOr(i * 6 + 1, 0.0D),
                tag.getDoubleOr(i * 6 + 2, 0.0D),
                tag.getDoubleOr(i * 6 + 3, 0.0D),
                tag.getDoubleOr(i * 6 + 4, 0.0D),
                tag.getDoubleOr(i * 6 + 5, 0.0D)
            );
        }
    ...
    public static @Nullable Mesh2D readOldMeshFromTagNonNormalized(
        ...
        for (int i = 0; i < triangleNum; i++) {
            mesh.addTriangle(
                tag.getDoubleOr(i * 6 + 0, 0.0D) / halfWidth,
                tag.getDoubleOr(i * 6 + 1, 0.0D) / halfHeight,
                tag.getDoubleOr(i * 6 + 2, 0.0D) / halfWidth,
                tag.getDoubleOr(i * 6 + 3, 0.0D) / halfHeight,
                tag.getDoubleOr(i * 6 + 4, 0.0D) / halfWidth,
                tag.getDoubleOr(i * 6 + 5, 0.0D) / halfHeight
            );
        }
